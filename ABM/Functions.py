...
def make_agent_nodes(agent_list):
    ...
    This function is necessary for the initialization of the Model class.
    ...
    agent_nodes : List of tuples with agent ids and agents
        Ex [(0,{'agent':UserAgent}),(1,{'agent':InfluencerAgent}),...]
    '''
...
def make_agents_connections(agent_list, user_network, influencer_network, f_network_mult_factor, homophily_weight_range):
    """
    A function that takes a list of agents and then make appropiate connections between UserAgents and InfluencerAgent as an edge relationship.
    ...
    influencer_network : int
        Increase the degree of connections between InfluencerAgents and UserAgents.
    f_network_mult_factor: in
        Furtherly increases the degree of connections for the Finfluencer and not for the Rinfluencer
...
# ==============================================================================================
# The following methods are merely for testing and are not needed for the execution of the model
# ==============================================================================================
def eai(df_agents,agent_id,timestep):
    """
    Retrieves information from a specific agent at a given timestep in a pandas DataFrame.

    Parameters
    ----------
    df_agents : pandas DataFrame
        Takes a pandas DataFrame as input.
    agent_id : int
        Id of an agent in a pandas DataFrame generated by the Model
    timestep : int
        Specify a timestep in a pandas Dataframe generated by the Model
...
def iso_agent(df,aid):
    """
    Retrieves all information throughout all the timestep for a given agent.

    Parameters
    ----------
    df : pandas DataFrame
        Takes a pandas DataFrame generated by the Model.
    aid : int
        Specify an agent id to make a subframe for.
...
def calc_var(dfi):
    """
    A function to calculate how much userAgent's opinions varies over certain timesteps
    A high value is most likely equal to some error.
    ...
    -------
    df : pandas DataFrame
        Returns a pandas DataFrame with various information.
...
def normalDistNP(size,mean = 0, std = 15):
    """
    Calculates a normal distribution of values.
    Used to initialize userAgents with normal distributed values.
    ...
    std : std, optional
        specify the standard deviation. The default is 15.
